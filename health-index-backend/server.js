...
const fs = require('fs'); // NEW: Import file system module for checks
...
// --- Mongoose Model (No Change) ---
const Submission = require('./models/Submission'); 

// --- 1. AUTHENTICATION MIDDLEWARE (No Change) ---
...
// ---------------------------------
// --- 2. ROUTES (API Endpoints) (No Change) ---
// ---------------------------------
...
// ----------------------------------
// --- 3. PRODUCTION FRONTEND SERVING (UPDATED) ---
// ----------------------------------

// Check if we are in production and need to serve the static files
if (process.env.NODE_ENV === 'production' || process.env.SERVE_FRONTEND === 'true') {
    // Determine the root of the app (which is /workspace)
    const appRoot = path.resolve(__dirname, '..'); 
    
    // The expected path to the built frontend files
    // Based on your monorepo structure (root/health-index-frontend/dist)
    const frontendPath = path.join(appRoot, 'health-index-frontend', 'dist');
    const indexPath = path.resolve(frontendPath, 'index.html');

    console.log(`[FRONTEND_PATH_CHECK] App Root: ${appRoot}`);
    console.log(`[FRONTEND_PATH_CHECK] Expected Static Directory: ${frontendPath}`);
    console.log(`[FRONTEND_PATH_CHECK] Expected Index File: ${indexPath}`);
    
    // Check if the output directory exists
    if (!fs.existsSync(frontendPath)) {
        console.error(`[FRONTEND_ERROR] Static frontend directory NOT found at: ${frontendPath}`);
        console.error(`[FRONTEND_ERROR] Cannot GET / will persist. Is the build command correct?`);
    } else if (!fs.existsSync(indexPath)) {
        console.error(`[FRONTEND_ERROR] index.html NOT found at: ${indexPath}`);
    } else {
        console.log('[FRONTEND_SUCCESS] Static directory and index.html found. Serving frontend.');
        
        // Serve any static files (JS, CSS, images, etc.)
        app.use(express.static(frontendPath));

        // Catch all other GET requests (the main '/' route and client-side routes)
        app.get('*', (req, res) => {
            res.sendFile(indexPath);
        });
    }
} else {
    console.log('[FRONTEND_SKIP] Not in production environment. Skipping static file serving.');
}
